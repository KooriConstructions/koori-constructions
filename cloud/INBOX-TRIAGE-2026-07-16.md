# Koori Inbox Triage & Draft Pack — 2026-07-16

**Scope:** brad@kooriconstructions.com.au · last 30 days · **953 inbox / 375 sent**.
**Covered in depth here:** the most recent ~75 (where unactioned items cluster). The
long tail is to be swept by the standing AI-admin runner (see `koori_email_admin.py`).
**Rule applied:** draft replies for human-needed mail; FYI/action flags for
bills/compliance; marketing → unsubscribe list (no auto-clicking).

Nothing in this file has been sent. Drafts are ready for you to review + send.

---

## 🔴 ACTION TODAY (hard deadlines)

### 1. Awabakal Medical Centre — vinyl flooring DEPOSIT DUE TODAY
Newcastle Flooring (Brittany, brittany@newcastleflooring.com.au / 0447 601 850)
**accepted quote 10935**, install **booked Tue 21 & Wed 22 Jul** — but needs the
**deposit paid today/tomorrow (15–16 Jul)** to hold that date. This is a **payment
action**, not just a reply.
- **Do:** pay the deposit in Xero now → send remittance. Draft ack below.

### 2. Thrumster security screens — On Time Blinds needs a measurement answer
Guy Dein (On Time Blinds, admin@ontimeblinds.com.au) sent quote **QU13684.pdf** and
asks: RFQ said front door **820–920mm**, plan shows **1200mm** — which is right? He
can't finalise until you confirm. Screens are the **critical path** to the 3 Aug
Thrumster completion.
- **Do:** confirm the door width + ask his firm fabrication/install lead time (must
  hit Wed 29 Jul). Draft below. ⚠ You must supply the correct width — I can't verify it.

### 3. Thrumster site access — AHO PM is out of office until Mon 20 Jul
Desmond Madden auto-replied OOO till 20/07. Your urgent measure-access request won't
be seen till then, but you need the measure THIS week. Escalation he listed:
**tony.todorovski@aho.nsw.gov.au** (urgent acquisitions).
- **Do:** chase Tony today. Draft below.

### 4. Darkinjung / Ray White — preferred-contractor onboarding + live job
Tabatha Buist (Ray White Thompson Partners, tabatha.buist@raywhite.com): Darkinjung
Aboriginal Land Council wants **Koori as preferred contractor for handyman work across
their portfolio.** Needs your: PL insurance, ABN, business address, owner's licence,
bank details. PLUS an immediate job: privacy screens at **17 Menindee Ave, Blue Haven**
— contact tenant **Megan Cain 0432 140 315 / megscain23@gmail.com** for a quote.
- **Do:** reply + attach onboarding docs; call/quote the tenant. Draft below.

---

## 🟠 ACTION THIS WEEK (no reply sent yet)

| # | From | Re | What's needed |
|---|------|----|----|
| 5 | Michelle (shelchis@bigpond.net.au) | Mallabula bathroom | She's still interested, asked for a **call tomorrow 1–2pm**. Confirm the slot. |
| 6 | Natsuko (natskit@hotmail.com) | Q1167 Deck & Pergola | Sent colour selections (Stratco Outback snowdrift/mountain blue; Ekodeck Backbeach). Acknowledge + confirm next step/order. |
| 7 | EstimateOne / Rogers Construction | **Muswellbrook High School** internal refurb tender | Respond QUOTING / NOT QUOTING. Decision needed. |
| 8 | Ann Marie Tidyman (Stratco) | Leads 204045 & 204154 | Chasing whether site visits booked + notes. Update the leads / reply. |
| 9 | Joss Group (Anthony Carlyon) | Catholic Diocese tender | ✅ You already replied "we'd love to price it." No action. |

---

## 🟡 FYI / INTERNAL FLAGS (no reply, but note)

- **⚠ Anthropic API spend hit your $1,400 monthly cap — further API use paused.** This
  likely stops your own automations (recipe miner / QS-3 / this AI admin) until you
  raise the limit. Operational — worth deciding on.
- Central Coast Bricks invoice #40007 ($421) — shows **paid** (remittance sent). FYI.
- icare NSW Workers Comp July newsletter — keep (compliance-relevant), don't unsub.
- ⚠ **Possible phishing — do NOT click:** "Furqan Open Ai / MetaAI GPT Ads" via Apple
  TestFlight; "Hacked Asset | Case 1199273993259477" from support.facebook.com. Verify
  independently; treat as suspicious.

---

## ✅ DRAFT REPLIES (review, then send)

### Draft 1 — Newcastle Flooring (Awabakal deposit)
> To: brittany@newcastleflooring.com.au
> Subject: RE: Quote no. 10935 — Accepted — Awabakal Medical Centre vinyl
>
> Hi Brittany,
> Thanks for holding the 21–22 July install. I'm processing the deposit today so we're
> locked in for those dates — I'll send the remittance through as soon as it's away.
> Anything you need from us on site access or subfloor prep beforehand, just let me know.
> Thanks,
> Brad Robinson · Koori Constructions

### Draft 2 — On Time Blinds (Thrumster screens)
> To: admin@ontimeblinds.com.au
> Subject: RE: RFQ — Security screens x3 — 7 Mackellar Way, Thrumster
>
> Hi Guy,
> Thanks for the quote (QU13684). On the front door — [CONFIRM: the correct width is
> ___mm; please amend the quote to suit]. The other two openings are per the scope/plan.
> The one thing that decides this for us is timing: can you confirm your firm fabrication
> + install lead time? We need all three installed by Wed 29 July. If that's achievable
> I'd like to lock you in and get you on site for a measure this week.
> Thanks,
> Brad Robinson · Koori Constructions

### Draft 3 — AHO escalation (Thrumster access)
> To: tony.todorovski@aho.nsw.gov.au; cc: procurement@aho.nsw.gov.au
> Subject: RFT-2015294 Thrumster — urgent site access for trade measures this week
>
> Hi Tony,
> Desmond Madden is on leave until 20 July and I need to keep RFT-2015294 (7 Mackellar
> Way, Thrumster) moving to hold the 3 August completion. Our electrician and security-
> screen fabricator need to measure on site this week. Could you confirm access
> arrangements (keys/lockbox — property is vacant) or point me to the right person while
> Desmond's away? Happy to work around you. WHS Management Plan is attached.
> Thanks,
> Brad Robinson · Koori Constructions · 0448 368 354

### Draft 4 — Ray White / Darkinjung (Tabatha) — FINALISED with her requested details
_Cross-checked Sent: no reply exists → genuinely outstanding. Details filled from company
records; two items only Brad can attach are marked [ATTACH]._
> **To:** tabatha.buist@raywhite.com
> **Subject:** RE: Quote request - 17 Menindee Avenue, Blue Haven NSW 2262
>
> Hi Tabatha,
>
> Thank you — we'd be glad to come on board as your preferred contractor for the
> Darkinjung Aboriginal Land Council portfolio. We're 100% Aboriginal-owned and this is
> exactly the kind of work we love to do.
>
> Here are our details for your system:
> • **Business name:** Koori Constructions Pty Ltd
> • **ABN:** 85 680 349 571
> • **NSW Builder Licence:** 477499C (Builder — Contractor)
> • **Business address:** 11 Brookfield Cl, Jilliby NSW 2259  _(confirm this is the address you want on file)_
> • **Public Liability:** certificate attached (Policy SPD014852046)
> • **Owner's driver's licence:** attached
> • **Bank account for invoicing:** Koori Constructions Pty Ltd · BSB 067-873 · Acct 1398 1227
>
> I'll contact the tenant, Megan Cain (0432 140 315), today to arrange a look at the
> privacy screens at 17 Menindee Ave and get a quote back to you shortly.
>
> Looking forward to working together.
> Brad Robinson · Director · Koori Constructions Pty Ltd · 0448 368 354
>
> **Attach before sending (2 files):**
> 1. **Public Liability Insurance** — new copy Brad supplied (save to SharePoint
>    `01_Company/Insurance-Certificates-of-Currency/`, then attach).
> 2. **Driver's licence** — the card photo Brad supplied (this is the "owner's driver's
>    licence" Tabatha asked for; save to the company licences folder, then attach).
>
> _All text details above are now complete (bank details included). The only reason this
> isn't already in your Drafts with both files attached: the Outlook/SharePoint connector
> here is read-only — no upload/draft/attach. Wire write access (#2) and I do this end-to-end._

### Draft 5 — Michelle / Mallabula
> To: shelchis@bigpond.net.au
> Subject: RE: Your renovation enquiry — Koori Constructions
>
> Hi Michelle,
> No worries at all. Tomorrow between 1–2pm works well — I'll give you a call then. If
> anything changes just let me know.
> Thanks, Brad

### Draft 6 — Natsuko / Q1167 Deck & Pergola
> To: natskit@hotmail.com
> Subject: RE: Colour selections — Q1167 Deck & Pergola
>
> Hi Natsuko,
> Thanks for the colour selections — Stratco Outback roof in Snowdrift White with
> Mountain Blue edges, and Ekodeck Backbeach for the deck and screening. That all works;
> I'll confirm stock/lead time and the next steps for scheduling and get back to you.
> Thanks, Brad

---

## 🧹 UNSUBSCRIBE LIST (for your approval — I have NOT clicked anything)
Marketing/newsletters seen in the recent window. Approve and I'll prepare the unsub
actions (or set Outlook rules to auto-file):
- Stratco EDM (reply@edm.stratco.com.au)
- RecruitShop (hello@recruitshop.com)
- Meetup (info@email.meetup.com; Sydney-Crowdfunding-Club-announce@email.meetup.com)
- American Express survey (americanexpress@feedbackemail.americanexpress.com)
- Total Tools (insider@email.totaltools.com.au)
- Fundrise (investments@fundrise.com)
- Goanna Academy (via hubspotstarter)
- Google Ads promo (ads-noreply@google.com)
- CBA marketing EDM (no-reply@edm.cba.com.au) — note: these look like transactional
  card/payment alerts; confirm before unsubbing.
- Supply Nation Connect events (connect@supplynation.org.au) — you may want to keep
  (Indigenous business network).

**Keep (do not unsub):** icare NSW, EstimateOne (tender leads), Xero, Anthropic.

---

# BATCH 2 — deeper triage (13–14 Jul, ~150 of 955 covered)

## 💰 Accepted quotes needing deposit invoices / next step (money on the table)
- **Awabakal Medical Centre vinyl (Q10935)** — accepted (see Batch 1, deposit due).
- **Natsuko — Q1167 Deck & Pergola** — quote digitally signed/accepted (Buildxact). Raise deposit + schedule.
- **Tori Burns — Q1139 Bathroom** — quote sent; chase for decision.

> ⚠️ **Correction (per Brad):** "**Accepted: Quote – ensuite – West Wallsend**" (terryanne71)
> and "**Accepted: Quote – Deck Wyee**" (dani.frazer) are **calendar RSVPs** — the client
> accepted the *quote/site-visit appointment*, NOT the price. So they're **booked quote
> visits to attend**, not deposits to raise. The engine classifier has been fixed to tell
> calendar RSVPs apart from real acceptances (Buildxact-signed / written "accepted").

## 🏗️ New opportunities / quotes to prepare (unactioned)
- **Awabakal — Glendale preschool floor plans** — Donna Smith (dsmith@awabakal.org, GM Preschools, 0403 717 209): "plans attached, call me anytime." Warm repeat client — reply + quote.
- **Precise Build — 47–49 Deane St, Belmont (Stratco outback awnings ×2)** — Tim sent plans for a quote. Prepare.
- **33 Cowper St, Wallsend (Stratco awning)** — Simon Entwistle (Stratco) supplied lineal-metre pricing ($48/lm). You're mid-quote; finish it.
- **Tayla — Q1136 Laundry Reno** — client's MIL wants laundry + bathroom measurements before deciding. Send measurements.

## ⚖️ Liability / legal — watch closely
- **738 Jilliby Rd — "Damage to Kitchen Cabinetry – Notice of Liability & Request for Indemnification"** (via ServiceM8 / Levant Central, Portia Brown, WO 9318). You've responded and requested particulars; Portia will revert. Keep a paper trail — this is a formal claim.

## 📑 Tenders needing a QUOTING / NOT-QUOTING decision
- **Muswellbrook High School – Internal Refurbishments** (Rogers Construction, via EstimateOne).
- **Greta Central Oval** (Kingston Building, via EstimateOne).

## 🤖 Your existing AI fleet (JARVIS) — noise + signal
- **[JARVIS] digests #17–29** report "8 employees running, ~20–107 artifacts/day" and route "decisions waiting" to you by email. So your fleet IS producing — but its digests + the QA-test self-leads (`qa-test@`, `KC-2026-…`) + marketing are burying real client mail. **Recommendation:** the Email Admin should (a) auto-file JARVIS digests to a "Fleet" folder, (b) suppress QA/test leads, so your inbox shows only real humans.

## Coverage note
Triaged the most-recent **~150 of 955** inbox emails (back to ~13 Jul). The remaining
~800 are, on a consistent read, the same three noise buckets (JARVIS digests, QA-test
self-leads, marketing) plus occasional client mail. The standing Email Admin will sweep
the full tail once activated; I can also keep hand-triaging deeper on request.

## Extra draft replies
### Draft 7 — Awabakal / Glendale (Donna Smith)
> To: dsmith@awabakal.org
> Subject: RE: Glendale floor plans
> Hi Donna, thanks for sending the Glendale plans through. I'll review them and come back
> to you with any questions and a quote shortly — I'll give you a call this week too.
> Thanks, Brad Robinson · Koori Constructions

# BATCH 3 — deeper triage (13 Jul, ~200 of 957 covered)

## 🏗️ New leads / quotes to prepare (no reply sent yet)
- **Bindelong Corp — Ashley (bindelongcorp@gmail.com, 0432 399 657)** — RFQ: internal walls
  + powerpoints at their work depot/office, **Boolaroo**. Commercial. Reply + quote.
- **James Ahoy (bubbajra@yahoo.com.au, 0407 470 965)** — quote/site inspection: extra room +
  ensuite + open-plan living/kitchen **extension**. Reply + book inspection.
- **Katie Carlisle (katie.carlisle13@gmail.com)** — "Plans for 7 Quinton Close" attached (deck
  done; pool later). Reply + quote from plans.
- **Stratco lead #205005** — new allocated lead; ACCEPT in Stratco portal + action.

## 🔧 Supplier questions (cross-checked against Sent)
- ~~Green Point windows — which to cancel?~~ **✅ ALREADY REPLIED** (Brad, 13 Jul: "cancel the
  850×900mm window"). No action — corrected after Sent cross-reference.
- **Central Coast Bricks** — offered Hampshire (Red) / Kent (Cream) as closest match for
  Green Point; confirm quantity + colour to order. _(verify against Sent before drafting)_

> **Reply-status rule now applied:** every item is cross-checked against the Sent folder
> before it's flagged as "needs reply", so no duplicate/flooding drafts. Confirmed
> unanswered this pass: Tabatha (Menindee), On Time Blinds (screen width), Bindelong,
> James Ahoy, Katie Carlisle (7 Quinton).

## 💵 FYI / flags
- **Anthropic API cap** — threshold alerts escalating ($1,000 → $1,100 → $1,400), API paused
  each time. This is throttling the fleet/automations repeatedly. Raise the cap.
- **Aussie Home Loans (Lucy Witchalls)** — borrowing-capacity update (~$466k OO / $550k inv).
  Personal finance thread; reply when you're ready.
- **WIN/The Block TV commercial** — Kelly Gamblin; Bart (sales) to call you. No action yet.

## Coverage
Triaged **~200 of 957** (back to ~13 Jul 00:00). Remaining ~757 are, on a consistent read,
the three noise buckets (JARVIS digests, QA-test self-leads `KC-2026-…`, marketing) plus
occasional client mail. Will keep going in passes until the admin is live.

---

### Draft 8 — Tayla / Q1136 (measurements)
> To: yourgirltayla@gmail.com
> Subject: RE: Q1136 – Laundry Renovations – Quote
> Hi Tayla, no problem — I'll get the laundry and bathroom measurements to you so your
> mother-in-law can see exactly what's covered for tiles etc. [CONFIRM: attach/insert
> measured dimensions] I'll send those through shortly.
> Thanks, Brad
